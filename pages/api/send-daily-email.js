...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4️⃣ For each document, get property info and send email reminder
    let sentCount = 0;
    for (const doc of docs) {
      ...
      if (!toEmail) {
        console.warn(`⚠️ No email address found for property ${property.name}`);
        continue;
      }
      ...
      if (isFakeEmail(toEmail)) {
        console.log(`⚠️ Skipping fake/placeholder email for property ${property.name}: ${toEmail}`);
        continue;
      }

      // 5️⃣ Send the email
      try {
        const emailFrom = process.env.EMAIL_FROM || process.env.EMAIL_USERNAME || process.env.GMAIL_USER || process.env.SMTP_USER;
        ...
        // Continue with other documents even if one email fails
      }
    }

    return res.status(200).json({
      success: true,
      message: `Found ${docs.length} expiring documents. ${sentCount} email reminder(s) sent successfully.`,
