...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fileName = `lender_questionnaire_completed_${timestamp}_${sanitizedName}`;
    const filePath = `lender_questionnaires/${applicationId}/completed/${fileName}`;
    ...
    return res.status(200).json({
      success: true,
      message: 'Completed lender questionnaire uploaded successfully',
      filePath: filePath,
    });
  } catch (error) {
