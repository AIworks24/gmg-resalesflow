...
export default async function handler(req, res) {
  ...
  try {
    console.log('🔄 Starting comprehensive application types migration...');

    // First, check if application_types table exists
    ...
    if (!tableExists) {
      console.log('📋 Application types table does not exist, creating it first...');
    } else {
      console.log('✅ Application types table exists, updating with property-based types...');
    }
...
COMMENT ON COLUMN applications.application_type IS 'Property-based application type: single_property, multi_community, settlement_va, settlement_nc, public_offering';
    ...
    console.log('✅ Comprehensive application types migration completed successfully');
    console.log('New application types:', newTypes);
    console.log('Sample applications:', applications);

    res.status(200).json({ 
      success: true, 
      message: 'Comprehensive application types migration completed successfully',
      details: 'Created/updated application types to property-based system',
      applicationTypes: newTypes,
      sampleApplications: applications
    });

  } catch (error) {
    console.error('Error running comprehensive application types migration:', error);
    res.status(500).json({ 
