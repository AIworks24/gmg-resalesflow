 ...
 */
export const getServerStripe = (req = null) => {
  // Check if test mode is enabled from request (defaults to false = LIVE mode)
  const useTestMode = req ? getTestModeFromRequest(req) : false;
  const keys = getStripeKeys(useTestMode);
  const mode = getStripeMode(useTestMode);
