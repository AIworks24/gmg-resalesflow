...
export const getEmailDocumentOrder = () => {
  ...
  if (customOrder) {
    try {
      // Try to parse as JSON array first (more readable format)
      if (customOrder.trim().startsWith('[')) {
        const parsed = JSON.parse(customOrder);
        if (Array.isArray(parsed)) {
          return parsed.map(key => String(key).trim());
        }
      }
      
      // Fallback to comma-separated list (backward compatible)
      return customOrder.split(',').map(key => key.trim());
    } catch (error) {
      console.warn('Invalid DOCUMENT_SEND_ORDER format, using default order:', error.message);
    }
  }
