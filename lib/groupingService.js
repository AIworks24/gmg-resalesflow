  ...
  try {
    ...
    for (const group of groups) {
      if (group.property_owner_email) {
        if (!managerGroups[group.property_owner_email]) {
          managerGroups[group.property_owner_email] = [];
        }
        managerGroups[group.property_owner_email].push(group);
      }
    }

    // Send email to each unique property manager
    for (const [email, managerGroups] of Object.entries(managerGroups)) {
      try {
