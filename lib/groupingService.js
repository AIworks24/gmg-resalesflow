...
async function createPropertyGroups(applicationId, primaryProperty, linkedProperties) {
  try {
    ...
    // Idempotent: if groups already exist, return them (avoids unique_application_property violation)
    const { data: existingGroups } = await supabase
      .from('application_property_groups')
      .select('*')
      .eq('application_id', applicationId);
    if (existingGroups && existingGroups.length > 0) {
      console.log(`Property groups already exist for application ${applicationId} (${existingGroups.length} groups), returning existing`);
...
/**
 * Creates per-group property owner forms for a multi-community application.
 * Idempotent — skips forms that already exist for a given group + form_type.
 * @param {number|string} applicationId
 * @param {Array<{id: number, property_name: string}>} groups
 */
async function createPropertyOwnerFormsForGroups(applicationId, groups) {
  const { getApplicationTypeData } = require('./applicationTypes');
  ...
  try {
    ...
    if (appError || !application) {
      console.error(`[createPropertyOwnerFormsForGroups] Could not fetch application ${applicationId}:`, appError?.message);
      return;
    }

    // Pass groupingService's service-role client — getApplicationTypeData otherwise
    // falls back to the browser-only supabase proxy, which throws server-side.
    ...
    for (const group of groups) {
      for (const formType of requiredForms) {
        ...
        if (existing) {
          console.log(`[createPropertyOwnerFormsForGroups] Skipping ${formType} for group ${group.id} — already exists`);
          continue;
        }
      ...
      console.log(`[createPropertyOwnerFormsForGroups] Processed group ${group.id} (${group.property_name || ''})`);
    }

    console.log(`[createPropertyOwnerFormsForGroups] Done for application ${applicationId}, ${groups.length} group(s)`);
  } catch (error) {
    console.error('[createPropertyOwnerFormsForGroups] Error:', error);
  }
}
