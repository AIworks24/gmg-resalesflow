...
function fillPDFFields(pdfDoc, fields) {
  try {
    ...
    fields.forEach(fieldData => {
      ...
      // Try alternative field name for Check Box11 (with/without period)
      if (!pdfField && fieldName === 'Check Box11.') {
        pdfField = fieldMap.get('Check Box11');
        if (pdfField) fieldType = pdfField.constructor.name;
      ...
      if (!pdfField && fieldName.match(/^Amount DueRow(\d+)$/)) {
        ...
        pdfField = fieldMap.get(`Amount Due Row${rowNum}`);
        if (!pdfField) pdfField = fieldMap.get(`AmountDueRow${rowNum}`);
        if (pdfField) fieldType = pdfField.constructor.name;
      }
      ...
      try {
        ...
        if (fieldType === 'PDFTextField') {
          ...
          const isApp14TextField = /^insurance_Description0\d+$/.test(fieldName) ||
                                   /^Text-App14-0\d+$/.test(fieldName);
          
          try {
            if (isApp14TextField) {
              // Enable auto-fit so long user-entered text shrinks to fit the field bounds
              textField.enableAutoFit();
              textField.setFontSize(0); // 0 = variable/auto size per PDF spec
            } else {
