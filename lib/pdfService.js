...
function fillPDFFields(pdfDoc, fields) {
  ...
  try {
    ...
    fields.forEach(fieldData => {
      ...
      // Special handling: cb4.1 and cb4.2 are now checkboxes (converted from radio buttons)
      if (fieldName === 'cb4.1' || fieldName === 'cb4.2') {
        // These are checkboxes, handle them in the checkbox section below
        // Don't skip here, let it fall through to checkbox handling
      }
      ...
      try {
        ...
        if (fieldType === 'PDFTextField') {
          ...
          // Debug logging for new Appendix 4 checkbox fields
          const isApp4Checkbox = fieldName === 'cb4.1' || fieldName === 'cb4.2';
          if (isApp4Checkbox) {
            console.log(`[PDF Service] Processing Appendix 4 checkbox field:`);
            console.log(`  - Requested field name: "${fieldName}"`);
            console.log(`  - Actual PDF field name: "${checkBox.getName()}"`);
            console.log(`  - fieldData.value:`, fieldData.value, `(type: ${typeof fieldData.value})`);
            console.log(`  - fieldData.text:`, fieldData.text, `(type: ${typeof fieldData.text})`);
          ...
          try {
            if (isChecked) {
              ...
              if (isApp4Checkbox) {
                console.log(`[PDF Service] ✓✓✓ Appendix 4 checkbox "${fieldName}" CHECKED successfully! ✓✓✓`);
                console.log(`[PDF Service]   - Field "${checkBox.getName()}" is now checked`);
              }
            } else {
              checkBox.uncheck();
              ...
              if (isApp4Checkbox) {
                console.log(`[PDF Service] ✗ Appendix 4 checkbox "${fieldName}" UNCHECKED`);
              }
            }
          } catch (error) {
            if (fieldName === 'Check Box11.' || fieldName === 'Check Box11') {
