...
function mapFormDataToPDFFields(formData) {
  ...
  const updatedFields = relevantFields.map((field) => {
    ...
    if (typeof mapper === "function") {
      try {
        const mappedValue = mapper(formData);
        text = mappedValue != null ? String(mappedValue) : "";
      } catch (error) {
        console.error(`Error mapping field ${fieldName}:`, error);
        text = "";
      }
    } else if (typeof mapper === "string") {
      text = formData[mapper] != null ? String(formData[mapper]) : "";
    }
