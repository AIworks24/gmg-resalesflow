...
function fillPDFFields(pdfDoc, fields) {
  ...
  try {
    ...
    fields.forEach(fieldData => {
      ...
      // Special handling: Group_Appx4.1.Choice1 might be a checkbox, try to find it as checkbox first
      if (fieldName === 'Group_Appx4.1.Choice1') {
        const checkBoxField = fieldMap.get(fieldName);
        if (checkBoxField && checkBoxField.constructor.name === 'PDFCheckBox') {
          // It's a checkbox! Handle it here
          ...
          try {
            ...
            if (isChecked) {
              checkBox.check();
              console.log(`[PDF Service] ✓ Group_Appx4.1.Choice1 CHECKED (as checkbox)`);
            } else {
              checkBox.uncheck();
              console.log(`[PDF Service] ✓ Group_Appx4.1.Choice1 UNCHECKED (as checkbox)`);
            }
            return; // Skip radio group handling - we handled it as checkbox
          } catch (error) {
            console.log(`[PDF Service] Group_Appx4.1.Choice1 is not a checkbox, will try as radio button`);
            // Fall through to radio group handling
          }
        }
        // If not a checkbox, fall through to radio group handling below
      }
      ...
      if (!pdfField) {
        ...
        // Debug: Log if Appendix 4 fields are not found
        if (fieldName === 'Group_Appx4.1.Choice1') {
          console.log(`[PDF Service] ❌ Appendix 4 Association levies assessments field NOT FOUND in PDF template!`);
          console.log(`  - Looking for field name: "${fieldName}"`);
          console.log(`  - Available fields containing "Appx4" or "App4":`, 
            Array.from(fieldMap.keys()).filter(k => k.includes('Appx4') || k.includes('App4')));
        }
        return;
      }
      ...
      // Debug: Log when Appendix 4 fields are found
      if (fieldName === 'Group_Appx4.1.Choice1') {
        console.log(`[PDF Service] ✓ Appendix 4 Association levies assessments field FOUND in PDF template`);
        console.log(`  - Field type:`, fieldType);
        console.log(`  - Field name in data:`, fieldName);
        console.log(`  - Actual PDF field name:`, pdfField.getName());
        console.log(`  - Field value:`, fieldData.value, `(text: ${fieldData.text})`);
