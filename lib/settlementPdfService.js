...
/**
 * Load settlement PDF template from local filesystem
 * @param {string} propertyState - 'VA' or 'NC'
 * @returns {Promise<Uint8Array>} - PDF template bytes
 */
async function loadSettlementTemplate(propertyState) {
  ...
  try {
    // Template file path - using the combined template file
    const templatePath = 'assets/Dues Request-Escrow Instructions (VA)_Statement of Unpaid Assessments (NC) (2).pdf';
...
async function generateSettlementPDF(formData, propertyState, applicationId) {
  try {
    ...
    const templateBytes = await loadSettlementTemplate(propertyState);
