...
const VA_FIELD_MAPPINGS = {
  ...
  "Unit Number": (data) => (data.unitNumber && data.unitNumber !== 'N/A') ? data.unitNumber : '',
  ...
  // Form completion (Month-Day-Year e.g. 1-23-2026)
 ...
 */
function prepareSettlementFormData(application, hoaProperty, accountantUser, timezone) {
  ...
  const baseData = {
    ...
    // Closing Information (Month-Day-Year e.g. 1-23-2026)
    // Uses regex-first approach to avoid UTC midnight timezone shift on date-only strings.
    estimatedClosingDate: application.closing_date
      ? (() => {
          const m = typeof application.closing_date === 'string' && application.closing_date.match(/^(\d{4})-(\d{2})-(\d{2})/);
          if (m) return `${parseInt(m[2], 10)}-${parseInt(m[3], 10)}-${m[1]}`;
          const d = new Date(application.closing_date);
          if (isNaN(d.getTime())) return 'TBD';
          const tz = timezone || 'UTC';
          const str = d.toLocaleDateString('en-US', { year: 'numeric', month: 'numeric', day: 'numeric', timeZone: tz });
          return str.replace(/\//g, '-');
        })()
      : 'TBD',
...
async function generateMultiCommunitySettlementPDFs(application, allProperties, accountantUser, timezone) {
  ...
  const pdfPromises = allProperties.map(async (property, i) => {
    ...
    try {
      ...
      const formData = prepareSettlementFormData(application, property, accountantUser, timezone);
