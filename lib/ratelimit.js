...
export async function checkRateLimit(identifier, limiterType) {
  // Fail open if Redis is not available — rate limiting is a best-effort guard,
  // not a hard gate. Blocking all requests when Redis is down would break core
  // auth flows (password reset, email confirmation) for all users.
  if (!redis) {
    console.warn('[RateLimit] Redis not configured - rate limiting disabled, allowing request');
    return {
      success: true,
      remaining: 999,
