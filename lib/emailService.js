...
export const sendEmailConfirmationEmail = async ({
  ...
  try {
    ...
    // Embed logo as base64 data URI for maximum email client compatibility
    // This avoids email clients blocking external images
    ...
    try {
      ...
      if (fs.existsSync(logoPath)) {
        ...
        console.log(`[EmailService] ✅ Logo embedded as base64 data URI (${Math.round(base64.length / 1024)}KB)`);
      } else {
        ...
        if (fs.existsSync(assetsLogoPath)) {
          ...
          const base64 = fileBuffer.toString('base64');
          logoDataUri = `data:image/png;base64,${base64}`;
          console.log(`[EmailService] ✅ Logo embedded from assets folder as base64 data URI (${Math.round(base64.length / 1024)}KB)`);
        } else {
          console.warn(`[EmailService] Logo file not found at: ${logoPath} or ${assetsLogoPath}`);
        }
      }
    } catch (error) {
    ...
    // Use base64 data URI for logo - most reliable method across all email clients
    // This works for both Microsoft Graph and SMTP without external dependencies
    const logoImg = logoDataUri 
      ? `<img src="${logoDataUri}" alt="GMG ResaleFlow" width="200" height="60" border="0" style="height:60px;width:auto;max-width:200px;display:block;margin:0 auto;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" />`
      : `<div style="color:#ffffff;font-size:20px;font-weight:600;text-align:center">GMG ResaleFlow</div>`;
