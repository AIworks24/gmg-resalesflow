...
export const sendInvoiceReceiptEmail = async ({
  ...
  try {
    ...
    // Get credit card logo/brand display
    const getCardBrandDisplay = (brand) => {
      if (!brand) return '';
      const brandUpper = brand.toUpperCase();
      // Use brand name with styling - email clients have limited SVG support
      const brandColors = {
        'VISA': '#1A1F71',
        'MASTERCARD': '#EB001B',
        'AMEX': '#006FCF',
    ...
    const logoUrl = 'https://dnivljiyahzxpyxjjifi.supabase.co/storage/v1/object/public/bucket0/assets/company_logo_white.png';
    const brandColor = '#0f4734';
    const brandColorLight = '#1a5f47';
    
    const subject = `Payment Receipt #${invoiceNumber} - Application #${applicationId}`;
    const html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
export const sendEmailConfirmationEmail = async ({
  ...
  try {
    ...
    // Use Supabase storage URL for logo to reduce email size and prevent Gmail clipping
    // Base64 embedding was causing emails to exceed Gmail's ~102KB clipping threshold
    const logoUrl = 'https://dnivljiyahzxpyxjjifi.supabase.co/storage/v1/object/public/bucket0/assets/company_logo_white.png';
    ...
    // Using external URL instead of base64 to significantly reduce email size
    const logoImg = `<img src="${logoUrl}" alt="GMG ResaleFlow" width="200" height="60" border="0" style="height:60px;width:auto;max-width:200px;display:block;margin:0 auto;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic" />`;
