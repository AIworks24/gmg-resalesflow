 ...
 */
export const sendEmail = async ({ to, subject, html, from, attachments = [], context = '', cc = [] }) => {
...
export const sendApplicationSubmissionEmail = async ({
  ...
  try {
    ...
    if (packageType === 'rush') {
      // For rush, calculate 5 business days
      let daysAdded = 0;
      while (daysAdded < 5) {
      ...
      // For standard, use 15 calendar days (including weekends)
    ...
    const getApplicationTypeTerm = (type) => {
      if (type === 'info_packet') {
        return 'Info Packet (Welcome Package) order';
    ...
    const getRequestTypeTerm = (type) => {
      if (type === 'info_packet') {
        return 'Info Packet (Welcome Package)';
      } else if (type === 'lender_questionnaire') {
    ...
    // "Application Type" in email = Requestor type (Builder, Realtor, Settlement), same as "Type" in application modal
    const formatRequestorType = (type) => {
      if (!type) return 'N/A';
    ...
    const applicationTypeDisplay = formatRequestorType(submitterType) || getApplicationTypeDisplay(applicationType, hasLinkedProps);
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5; line-height: 1.6; color: #333333;">
  ...
  <div class="email-container" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    ...
    <div class="email-content" style="padding: 30px 20px; background-color: #ffffff;">
      ...
      <div class="email-card" style="background-color: #f9fafb; padding: 24px; border-radius: 8px; margin: 0 0 24px 0; border: 1px solid #e5e7eb;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb; font-size: 14px; color: #111827; font-weight: 500;">
              1. ${escapeHtml(hoaName)} (Primary Property)
            </td>
          </tr>
          ${linkedProperties.map((prop, index) => {
            ...
            return `
          <tr>
            <td style="padding: 8px 0; ${index < linkedProperties.length - 1 ? 'border-bottom: 1px solid #e5e7eb;' : ''} font-size: 14px; color: #111827;">
              ${index + 2}. ${escapeHtml(propName)}${propLocation ? ` (${escapeHtml(propLocation)})` : ''}
            </td>
          </tr>`;
          }).join('')}
        </table>
      ...
      <div class="what-happens-next-card" style="background-color: #f0f9ff; padding: 24px; border-radius: 8px; margin: 0 0 24px 0; border: 1px solid #bae6fd;">
        ...
        <table class="steps-table" role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width: 100%; border-collapse: collapse;">
          ...
          <tr>
            <td class="step-number-cell" style="padding: 0 0 12px 0; vertical-align: top; width: 40px;">
            ...
            <td class="step-text-cell" style="padding: 0 0 12px 12px; font-size: 15px; color: #1e293b; line-height: 1.6; vertical-align: top;">
              ${escapeHtml(step)}
            </td>
          </tr>
          `).join('')}
  ...
  <style>
    ...
    @media only screen and (max-width: 600px) {
      ...
      /* Preserve table layout for numbered steps */
      .what-happens-next-card {
        padding: 20px 16px !important;
      }
      .steps-table td {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px;">
            ...
            <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; border-bottom: 1px solid #eee;">${packageType === 'rush' ? 'Rush (5 business days)' : 'Standard (15 calendar days)'}</td>
                </tr>
                <tr>
            ...
            <div style="background-color: #EBF8FF; padding: 15px; border-radius: 8px; margin: 20px 0;">
              ...
              <p style="margin: 0;">${isRush ? 'Rush processing will be applied to all communities, ensuring faster completion.' : 'Standard processing will be applied to all communities, with completion expected within 15 calendar days.'}</p>
            </div>
...
export const sendPropertyManagerNotificationEmail = async ({
  ...
  linkedProperties = [],
  applicationType,
  submitterType, // Requestor type (Builder, Realtor, Settlement) - same as "Type" in application modal
  recipientPropertyName = null, // For MC: the specific property this recipient manages
  isPartOfMultiCommunityApplication = false, // When true, add "Your property is part of a Multi-Community Application" note
}) => {
  try {
    ...
    const applicationTypeDisplay = formatRequestorType(submitterType) || (() => {
      const formatApplicationTypeForEmail = (type, rush) => {
        if (type === 'settlement_va') return rush ? 'Settlement VA (Rush)' : 'Settlement VA';
        if (type === 'settlement_nc') return rush ? 'Settlement NC (Rush)' : 'Settlement NC';
        if (type === 'multi_community') return 'Multi-Community';
        if (type === 'lender_questionnaire') return rush ? 'Lender Questionnaire (Rush)' : 'Lender Questionnaire';
        if (type === 'public_offering') return rush ? 'Public Offering (Rush)' : 'Public Offering';
        return rush ? 'Single Property (Rush)' : 'Single Property';
      };
      return formatApplicationTypeForEmail(applicationType, isRush);
    })();
    ...
    const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px;">
            ...
            ${isPartOfMultiCommunityApplication ? `
            <div style="background-color: #DBEAFE; padding: 16px; border-radius: 8px; margin: 16px 0; border-left: 4px solid #2563EB;">
              <p style="margin: 0; font-weight: 600; color: #1E40AF;">📋 Your property is part of a Multi-Community Application</p>
              <p style="margin: 8px 0 0 0; font-size: 14px; color: #1E3A8A;">${recipientPropertyName ? `Your property <strong>${recipientPropertyName}</strong> ` : 'This application '}is included in a multi-community resale certificate request. Multiple community associations will be processed together for this application.</p>
            </div>
            ` : ''}
...
export const sendInfoPacketPropertyNotificationEmail = async ({
  to,
  applicationId,
  propertyName,
  propertyAddress,
  ...
  try {
    const normalizedTo = normalizeEmail(to);
    const subject = `Info Packet (Welcome Package) Order — ${propertyName}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #2563EB; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
          <h1 style="margin: 0;">Info Packet Order Received</h1>
          <p style="margin: 10px 0 0 0;">Goodman Management Group - ResaleFlow</p>
        </div>
        <div style="background-color: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px;">
          <p>Dear Property Manager,</p>
          <p>An <strong>Info Packet (Welcome Package)</strong> order has been placed for your property. Documents have been automatically delivered to the requester — no action is required from you.</p>
          <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                <td style="padding: 8px 0;"><strong>Requester:</strong></td>
                <td style="padding: 8px 0;">${submitterName} (${submitterEmail})</td>
              </tr>
            </table>
          </div>
          <div style="background-color: #DCFCE7; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #16A34A;">
            <p style="margin: 0; color: #15803D; font-weight: 600;">✅ No action required</p>
            <p style="margin: 8px 0 0 0; font-size: 14px; color: #166534;">The Welcome Package and available property documents were sent automatically upon payment completion.</p>
          </div>
          <div style="border-top: 1px solid #E5E7EB; padding-top: 20px; text-align: center; color: #6B7280; font-size: 12px;">
            <p>Goodman Management Group<br>Professional HOA Management &amp; Resale Services</p>
          </div>
        </div>
      </div>
    `;
    return await sendEmail({
      ...
      subject,
      html,
      context: `InfoPacketPropertyNotification-${applicationId}`
    });
  } catch (error) {
    console.error(`[EMAIL_SERVICE] App ${applicationId}: ✗ Failed to send info packet property notification to ${to}`, error.message);
    throw error;
  }
};
  ...
  <div class="email-container" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    ...
    <div class="email-content" style="padding: 30px 20px; background-color: #ffffff;">
      ...
      <div class="email-card" style="background-color: #f9fafb; padding: 24px; border-radius: 8px; margin: 0 0 24px 0; border: 1px solid #e5e7eb;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width: 100%; border-collapse: collapse;">
          ${lineItems.map(item => {
            // Display item name as title, description (with property address) as subtitle
            const itemTitle = escapeHtml(item.name);
            const isCreditCardFee = /credit card processing fee/i.test(item.name);
            const itemDescription = item.description ? escapeHtml(item.description) : null;
            const nonRefundableNote = isCreditCardFee ? '<div style="font-size: 12px; color: #6b7280; margin-top: 2px;">Non-refundable.</div>' : '';
            
            return `
          <tr>
            <td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-size: 14px; color: #374151;">
              <div style="font-weight: 600; color: #111827; margin-bottom: ${itemDescription || isCreditCardFee ? '4px' : '0'};">
                ${itemTitle}${item.quantity > 1 ? ` x ${item.quantity}` : ''}
              </div>
              ${itemDescription ? `<div style="font-size: 13px; color: #6b7280; margin-top: 2px;">${itemDescription}</div>` : ''}
              ${nonRefundableNote}
            </td>
