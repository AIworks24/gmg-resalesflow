...
export const sendEmailConfirmationEmail = async ({
  ...
  try {
    ...
    // Use public URL for logo - most reliable method across all email clients
    // Logo is now in public/company_logo_white.png and accessible via URL
    const logoUrl = `${baseUrl}/company_logo_white.png`;
    
    // Also prepare CID attachment as fallback for SMTP (some clients prefer this)
    ...
    try {
      ...
      if (fs.existsSync(logoPath)) {
        ...
        attachments.push({
          ...
          path: logoPath,
          cid: logoCid, // Content-ID for SMTP fallback
          contentType: 'image/png',
        });
        console.log(`[EmailService] ✅ Logo prepared - URL: ${logoUrl}, CID fallback: ${logoCid}`);
      } else {
        console.warn(`[EmailService] Logo file not found at: ${logoPath}, using URL only`);
      }
    } catch (error) {
      console.error('[EmailService] Error preparing logo attachment:', error.message);
      // Continue with URL-only approach
    }
    ...
    // Use public URL for logo - most reliable method across all email clients
    // CID attachment is still included for SMTP clients that prefer embedded images
