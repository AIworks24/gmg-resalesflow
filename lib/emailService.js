...
export const sendApplicationSubmissionEmail = async ({
  ...
  primaryPropertyComment = null, // Optional: comment for primary property in multi-community
}) => {
  try {
    // Normalize email to lowercase for consistent delivery
    const normalizedTo = normalizeEmail(to);
  ...
  <div class="email-container" style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
    ...
    <div class="email-content" style="padding: 30px 20px; background-color: #ffffff;">
      ...
      <div class="email-card" style="background-color: #f9fafb; padding: 24px; border-radius: 8px; margin: 0 0 24px 0; border: 1px solid #e5e7eb;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 8px 0; ${primaryPropertyComment ? '' : 'border-bottom: 1px solid #e5e7eb;'} font-size: 14px; color: #111827; font-weight: 500;">
              1. ${escapeHtml(hoaName)} (Primary Property)
              ${primaryPropertyComment ? `<div style="margin-left: 20px; margin-top: 6px; padding: 8px 12px; background-color: #f0fdf4; border-left: 3px solid #22c55e; border-radius: 4px; font-size: 13px; color: #15803d; font-style: italic; font-weight: normal;">${escapeHtml(primaryPropertyComment)}</div>` : ''}
            </td>
          </tr>
          ${primaryPropertyComment ? '<tr><td style="padding: 4px 0; border-bottom: 1px solid #e5e7eb;"></td></tr>' : ''}
          ${linkedProperties.map((prop, index) => {
            ...
            const comment = prop.relationship_comment;
            return `
          <tr>
            <td style="padding: 8px 0; ${index < linkedProperties.length - 1 && !comment ? 'border-bottom: 1px solid #e5e7eb;' : ''} font-size: 14px; color: #111827;">
              ${index + 2}. ${escapeHtml(propName)}${propLocation ? ` (${escapeHtml(propLocation)})` : ''}
              ${comment ? `<div style="margin-left: 20px; margin-top: 6px; padding: 8px 12px; background-color: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; color: #1e40af; font-style: italic;">${escapeHtml(comment)}</div>` : ''}
            </td>
          </tr>
          ${comment && index < linkedProperties.length - 1 ? '<tr><td style="padding: 4px 0;"></td></tr>' : ''}
          `;
          }).join('')}
        </table>
