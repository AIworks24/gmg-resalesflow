...
export const sendEmailConfirmationEmail = async ({
  ...
  try {
    ...
    // Vercel provides VERCEL_URL automatically, but we need to construct the full URL
    let baseUrl = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL;
    
    // If no explicit URL is set, try to use Vercel's automatic URL
    if (!baseUrl && process.env.VERCEL_URL) {
      baseUrl = `https://${process.env.VERCEL_URL}`;
    }
    
    // Fallback to placeholder if still not set
    ...
    const isLocalDev = baseUrl.includes('localhost') || baseUrl.includes('127.0.0.1') || baseUrl === 'https://your-app-url.com';
    ...
    if (isLocalDev) {
      ...
      console.log(`[EmailService] Base URL source: ${process.env.NEXT_PUBLIC_SITE_URL ? 'NEXT_PUBLIC_SITE_URL' : process.env.NEXT_PUBLIC_APP_URL ? 'NEXT_PUBLIC_APP_URL' : process.env.VERCEL_URL ? 'VERCEL_URL (auto)' : 'fallback'}`);
    }
