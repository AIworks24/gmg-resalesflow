...
export async function calculateMultiCommunityPricing(primaryPropertyId, packageType, applicationType, supabaseClient = null, submitterType = null, publicOffering = false, userId = null) {
  ...
  if (applicationType === 'public_offering' || applicationType === 'info_packet') {
...
// NOTE: This module is pure ESM (like lib/emailService.js). It is consumed both via
// `import { … }` and via `require('…/multiCommunityUtils')` from CommonJS API routes;
// Next.js/webpack's ESM↔CJS interop resolves named exports for both. A previous trailing
// `module.exports = { … }` block reassigned the exports of this harmony module, which
// corrupted the `require()` namespace inside the Stripe webhook's serverless bundle and
// caused `TypeError: s is not a function` in handleMultiCommunityApplication (every
// multi-community payment 500'd and Stripe retried). Do not re-add a manual module.exports.