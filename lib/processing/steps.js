...
import { getDeliveryPause } from '../propertyStatus';
import { appendProcessNote } from '../processHistoryServer';
import {
...
// Resolves the figure printed as "Total Amount Paid" on the receipt.
//
// This must be what Stripe actually charged, NOT applications.total_amount: for
// multi_community that column deliberately holds refundable service fees only, while
// checkout charges a $9.95 CC fee per association. Reading total_amount here understated
// every MC receipt by $9.95 x N while the Stripe line items printed below it listed each
// fee — the receipt did not add up (ClickUp 86d44v1by).
//
// Sources, most authoritative first. Each is a real Stripe figure until the last, which
// preserves the old behaviour when Stripe is unreachable and the column is unpopulated.
export function resolveReceiptTotal({ app, payload, chargedCents, lineItems }) {
  const candidates = [
    // 1. What Stripe settled on the charge.
    chargedCents != null ? chargedCents / 100 : null,
    // 2. The webhook's amount (session.amount_total, or the PaymentIntent amount).
    payload?.amountTotal != null ? payload.amountTotal / 100 : null,
    // 3. The amount the webhook persisted for this payment.
    app?.stripe_amount_total != null ? Number(app.stripe_amount_total) : null,
    // 4. Sum of the Stripe line items already fetched for the Summary card — guarantees
    //    the header total matches the breakdown shown underneath it.
    (lineItems || []).length > 0
      ? lineItems.reduce((sum, item) => sum + Number(item.amount), 0)
      : null,
    // 5. Last resort.
    app?.total_amount != null ? Number(app.total_amount) : null,
  ];

  const resolved = candidates.find(v => v != null && Number.isFinite(v));
  if (resolved == null) return '0.00';

  // Surface drift in the logs rather than in a customer's inbox.
  if (app?.total_amount != null && Math.abs(resolved - Number(app.total_amount)) > 0.01) {
    console.warn(
      `[Step:send_receipt] Application ${app.id}: charged $${resolved.toFixed(2)} but ` +
      `total_amount is $${Number(app.total_amount).toFixed(2)} — receipting the charged amount. ` +
      `(Expected for multi_community, where total_amount excludes per-property CC fees.)`
    );
  }
...
async function sendReceipt({ app, supabase, payload }) {
  ...
  const totalAmount = resolveReceiptTotal({ app, payload, chargedCents, lineItems });
...
async function deliverDocuments({ app, supabase, payload }) {
  if (app.email_completed_at) return { skipped: 'already_delivered' };

  // Property moved to Draft (files being updated) — don't auto-send possibly stale documents.
  // Recorded in Process History; an admin resends from the application once it is published.
  const pause = await getDeliveryPause(supabase, app.id);
  if (pause.paused) {
    await appendProcessNote(
      supabase,
      app.id,
      `Documents not delivered — property in Draft (${pause.draftPropertyNames.join(', ')}). Resend after publishing.`
    );
    return { skipped: 'property_draft', draftProperties: pause.draftPropertyNames };
  }
