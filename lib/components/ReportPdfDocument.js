 ...
 *   sections     {Array}    [{ title, columns, rows, colorKey?, repeatHeader? }]
 *                  colorKey:     fn(row) → 'red'|'orange'|'green'|'yellow'|null
 *                  repeatHeader: repeat the column headers on each page of a
 *                                long table (use on the LAST section only —
 *                                react-pdf `fixed` repeats document-wide)
...
function SectionTable({ section }) {
  const { title, columns, rows, colorKey, repeatHeader } = section;
  return React.createElement(
    View,
    { style: s.section },
    React.createElement(
    ...
    // Header row — `fixed` repeats it on every page of a table that spans pages
    React.createElement(
      View,
      { style: s.tableHeaderRow, fixed: !!repeatHeader },
      ...columns.map((col, ci) =>
        React.createElement(
          Text,
    ...
    ...rows.map((row, ri) => {
      ...
      // wrap:false keeps a row whole — without it a cell that wraps to two
      // lines can straddle a page break, orphaning half the row on the next page
      return React.createElement(
        View,
        { key: ri, style: rowStyle, wrap: false },
        ...columns.map((col, ci) => {
...
function ReportPdfDocument({ title, subtitle, period, generatedAt, logoBase64, kpis, sections, orientation = 'portrait' }) {
  return React.createElement(
    Document,
    null,
    React.createElement(
      Page,
      { size: 'A4', orientation, style: s.page },
      ...
      // Format before handing off — the footer prints whatever string it is
      // given verbatim, so a raw ISO timestamp would leak into the document
      React.createElement(ProfessionalFooter, { generatedDate: formatGeneratedAt(generatedAt) }),
    ),
  );
}
