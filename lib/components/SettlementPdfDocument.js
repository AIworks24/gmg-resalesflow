...
// Disable hyphenation to prevent hyphens when text wraps
...
const styles = StyleSheet.create({
  ...
  fieldFullWidth: {
    ...
    marginTop: 4,
    marginBottom: 6, // Increased bottom margin for better spacing between fields
    width: '100%',
  },
  label: {
    fontWeight: 'bold',
  ...
  value: {
    ...
    flex: 1,
    lineHeight: 1.4,
    // React PDF will wrap text naturally at word boundaries
    flexWrap: 'wrap',
  },
  valueLong: {
    color: '#111827',
  ...
  valueEmail: {
    ...
    flex: 1,
    lineHeight: 1.4,
    // React PDF will wrap email addresses naturally
    width: '100%',
  },
  textareaField: {
    marginTop: 0,
            ...
            section.title === 'Fees' && section.fields.length > 0 ? (() => {
              ...
              React.createElement(
                ...
                ...section.fields.map((field, fieldIdx) => {
                  ...
                  const isLongValue = valueLength > 50; // Lower threshold for long values
                  const isVeryLongValue = valueLength > 80; // Lower threshold - full width for values over 80 chars
                  const isEmptyValue = !field.value || String(field.value).trim() === '';
                  
                  // Detect email fields by label or key
                  const isEmailField = field.label?.toLowerCase().includes('email') || 
                                       field.key?.toLowerCase().includes('email') ||
                                       (field.value && /^[^\s@]+@[^\s@]+\.[^\s@]+/.test(String(field.value).split(',')[0].trim()));
                  
                  // Detect fields that commonly have long text and should be full width
                  const isLongTextField = field.label?.toLowerCase().includes('company') ||
                                         field.label?.toLowerCase().includes('title') ||
                                         field.label?.toLowerCase().includes('address') ||
                                         field.key?.toLowerCase().includes('company') ||
                                         field.key?.toLowerCase().includes('title') ||
                                         field.key?.toLowerCase().includes('address');
                  
                  // Email fields should be full width to prevent breaking with hyphens
                  if (isEmailField && !isEmptyValue) {
                    const emailValue = String(field.value);
                    const emails = emailValue.includes(',') 
                      ? emailValue.split(',').map(e => e.trim()).filter(e => e)
                      : [emailValue.trim()];
                    ...
                    return React.createElement(
                      ...
                      { 
                        ...
                        style: {
                          ...
                          marginBottom: 6, // Increased margin to prevent overlap with next field
                        }
                      },
                      React.createElement(
                        View,
                        ...
                        React.createElement(
                          ...
                          // Render each email on its own line to prevent breaking
                          ...emails.map((email, emailIdx) =>
                            React.createElement(
                              ...
                              { 
                                ...
                                style: { 
                                  ...styles.valueEmail, 
                                  marginTop: emailIdx === 0 ? 0 : 2,
                                  marginBottom: emailIdx === emails.length - 1 ? 0 : 2,
                                  width: '100%',
                                } 
                              },
                              email
                  ...
                  // Very long regular fields or long text fields should be full width to prevent hyphens
                  if (isVeryLongValue || (isLongTextField && isLongValue)) {
                    return React.createElement(
                      View,
                      { 
                        key: `field-${sectionIdx}-${fieldIdx}`, 
                        style: {
                          ...
                          paddingBottom: 4, // Increased padding for better spacing
                        }
                      },
                      React.createElement(
                        View,
