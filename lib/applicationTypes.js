...
export function determineApplicationType(submitterType, hoaProperty, publicOffering = false) {
  // Lender Questionnaire: NOT allowed for multi-community (MC is resale only; Public Offering is exception)
  if (submitterType === 'lender_questionnaire') {
    if (hoaProperty?.is_multi_community) {
      return 'single_property'; // MC + lender_questionnaire not supported; treat as single
    }
