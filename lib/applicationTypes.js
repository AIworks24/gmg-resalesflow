...
/**
 * Check if forced price should apply to an application type
 * Forced price only applies to standard resale applications (single_property, multi_community)
 * Does NOT apply to: settlement_va, settlement_nc, lender_questionnaire, public_offering
 * @param {string} applicationTypeName - The application type name
 * @returns {boolean} - True if forced price should apply
 */
export function shouldApplyForcedPrice(applicationTypeName) {
  ...
  return !excludedTypes.includes(applicationTypeName);
}
...
export async function getApplicationTypePricing(applicationTypeName, packageType, propertyId = null, supabaseClient = null) {
  // Check for forced price override if propertyId is provided AND forced price applies to this type
  if (propertyId && shouldApplyForcedPrice(applicationTypeName)) {
    try {
