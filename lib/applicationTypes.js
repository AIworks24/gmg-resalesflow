...
export function determineApplicationType(submitterType, hoaProperty, publicOffering = false, infoPacket = false) {
  ...
  if (submitterType === 'builder' && infoPacket) {
    return 'info_packet';
  }

  // Public Offering Statement (special case for builders - allowed for MC)
  if (submitterType === 'builder' && publicOffering) {
    return 'public_offering';
  }

  // Builder on NC property with no specific builder options → use NC settlement type (Statement of Unpaid Assessments)
  if (submitterType === 'builder' && hoaProperty) {
 ...
 * @param {Object} [supabaseClient] - Optional Supabase client. Pass a server-side
 *   (service-role) client when calling from API routes / the job worker; defaults to
 *   the browser client for backward compatibility with client-side callers. This is
 *   required server-side because the default `supabase` export is a browser-only proxy
 *   that throws ("Supabase client can only be accessed in browser context") on the server.
 * @returns {Promise<Object>} - Application type data
 */
export async function getApplicationTypeData(applicationTypeName, supabaseClient = supabase) {
...
export function getFormSteps(applicationTypeName) {
  if (applicationTypeName === 'lender_questionnaire') {
    // Lender Questionnaire flow: ends at payment. The lender's form is uploaded on
    // /payment/success, and that upload is what submits the application.
    return [
