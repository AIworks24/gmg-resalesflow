 ...
 * 
 * SECURITY: Only validates test mode via query parameter + secret code validation.
 * Does NOT trust request body to prevent client-controlled payment mode bypass.
 * 
...
function getTestModeFromRequest(req) {
  // Server-side: check query parameters with secret code validation
  ...
  // SECURITY: Removed request body check - never trust client-controlled test mode
  // This prevents malicious users from forcing test mode to avoid charges
