...
export const formatDate = (dateString) => {
  ...
  try {
    ...
    if (dateString instanceof Date) {
      ...
      // Date-only strings (YYYY-MM-DD) must NOT be parsed as midnight UTC —
      // that shifts the date back by one day in US timezones. Parse directly.
      const dateOnlyMatch = dateString.trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
      if (dateOnlyMatch) {
        const [, year, month, day] = dateOnlyMatch;
