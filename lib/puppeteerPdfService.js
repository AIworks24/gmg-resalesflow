...
async function htmlToPdf(htmlContent, options = {}) {
  ...
  try {
    ...
    if (isVercel) {
      // Get executable path - call without arguments to let chromium handle binary download
      // If CHROMIUM_REMOTE_EXEC_PATH is set, it will be used as the download URL
      // Otherwise, chromium will use its default download mechanism
      let chromiumPath;
      if (process.env.CHROMIUM_REMOTE_EXEC_PATH) {
        // If a custom URL is provided, pass it to executablePath
        chromiumPath = await chromium.executablePath(process.env.CHROMIUM_REMOTE_EXEC_PATH);
      } else {
        // Call without arguments - chromium will handle binary download automatically
