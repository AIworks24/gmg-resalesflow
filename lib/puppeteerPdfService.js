...
async function htmlToPdf(htmlContent, options = {}) {
  ...
  try {
    ...
    if (isVercel) {
      // Default Chromium binary URL for v131 (matches @sparticuz/chromium-min version)
      // This will be used if CHROMIUM_REMOTE_EXEC_PATH is not set
      const defaultChromiumUrl = process.env.CHROMIUM_REMOTE_EXEC_PATH || 
        'https://github.com/Sparticuz/chromium/releases/download/v131.0.0/chromium-v131.0.0-pack.tar.br';
      
      // Get executable path - this will download the binary from the URL
      const chromiumPath = await chromium.executablePath(defaultChromiumUrl);
