...
CHECK (role IN ('admin', 'staff', 'user', 'external', NULL));

-- Add a comment to document the rollback
COMMENT ON COLUMN profiles.role IS 'User role: admin (full access), staff (limited admin access), user (regular user), external (external user), or NULL (unassigned)';
